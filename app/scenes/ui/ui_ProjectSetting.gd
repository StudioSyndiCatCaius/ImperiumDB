...
func _ready():
	...
	if type==_type.BOOL:
		N_checkBox.toggle_mode=G_Project.dic_projects.get(Name,false)
		N_TypeRoot.get_child(0).visible=true
	if type==_type.INT:
		N_SpinBox.value=G_Project.dic_projects.get(Name,0)
		N_TypeRoot.get_child(1).visible=true


func _on_spin_box_value_changed(value):
	if type==_type.INT:
		G_Project.dic_projects[Name]=value

func _on_check_button_toggled(toggled_on):
	if type==_type.BOOL:
		G_Project.dic_projects[Name]=toggled_on
